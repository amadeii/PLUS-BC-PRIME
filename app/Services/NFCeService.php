...
class NFCeService
{
	...
	public function gerarXml($item)
	{
		...
		if ($cliente != null) {
			...
			$cpf_cnpj = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $cliente->cpf_cnpj));
			...
			if($cpf_cnpj != '00000000000'){
				...
				$stdEnderDest->xLgr = $this->retiraAcentos($cliente->rua);
				$stdEnderDest->nro = $this->retiraAcentos($cliente->numero);
				$stdEnderDest->xCpl = $this->retiraAcentos($cliente->complemento);
				$stdEnderDest->xBairro = $this->retiraAcentos($cliente->bairro);
				$stdEnderDest->cMun = $cliente->cidade->codigo;
				$stdEnderDest->xMun = strtoupper($this->retiraAcentos($cliente->cidade->nome));
		...
		if ($item->dinheiro_recebido > 0) {
			...
			if (sizeof($item->fatura) > 0) {
				foreach ($item->fatura as $ft) {
					...
					if(sizeof($item->fatura) == 1 && $vPag > 0){
						$stdDetPag->vPag = $vPag;
					}
	...
	public function transmitir($signXml, $chave)
	{
		try {
			...
			if (!in_array(($std->cStat ?? null), [103, 104])) {
				...
				return [
					'erro' => 1,
					'error' => '[' . ($std->cStat ?? '-') . '] - ' . ($std->xMotivo ?? 'Erro ao enviar lote'),
			...
			if (($std->cStat ?? null) == 103) {
				...
				if (($stdRet->cStat ?? null) != 104) {
					...
					return [
						'erro' => 1,
						'error' => '[' . ($stdRet->cStat ?? '-') . '] - ' . ($stdRet->xMotivo ?? 'Erro ao consultar recibo'),
						'cStat' => $stdRet->cStat ?? null,
						'recibo' => $recibo
					];
				}
			...
			file_put_contents(
				public_path('xml_nfce/') . $chave . '.xml',
				$xml
			);
