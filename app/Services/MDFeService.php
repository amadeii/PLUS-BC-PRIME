...
class MDFeService{
	...
	public function gerar($mdfe){
		...
		$cnpj = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $emitente->cpf_cnpj));
		...
		foreach($mdfe->ciots as $c){
			...
			$doc = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $c->cpf_cnpj));
		...
		foreach($mdfe->valesPedagio as $v){
			...
			$doc = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $v->cnpj_fornecedor_pagador));
		...
		// $doc = preg_replace('/[^0-9]/', '', $mdfe->veiculoTracao->proprietario_documento);
		$doc = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $mdfe->veiculoTracao->proprietario_documento));
		...
		if($mdfe->veiculo_reboque_id != NULL){
			...
			$doc = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $mdfe->veiculoReboque->proprietario_documento));
		...
		if($mdfe->veiculo_reboque2_id != NULL){
			...
			$doc = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $mdfe->veiculoReboque2->proprietario_documento));
		...
		if($mdfe->veiculo_reboque3_id != NULL){
			...
			$doc = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $mdfe->veiculoReboque3->proprietario_documento));
		...
		if($mdfe->nome_pagador && $mdfe->documento_pagador){
			...
			$doc = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $mdfe->documento_pagador));
		...
		if($mdfe->seguradora_cnpj != null){
			...
			$cnpj = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $mdfe->seguradora_cnpj));
		...
		if($mdfe->produto_pred_nome != ''){
			...
			if($mdfe->cep_carrega != ''){
				$cep = preg_replace('/[^0-9]/', '', $mdfe->cep_carrega);
				$localCarrega->CEP = $cep;
			}
			...
			if($mdfe->cep_descarrega != ''){
				$cep = preg_replace('/[^0-9]/', '', $mdfe->cep_descarrega);
				$localDescarrega->CEP = $cep;
			}
		...
		if($emitente->aut_xml != ""){
			...
			$cnpj = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $emitente->aut_xml));
