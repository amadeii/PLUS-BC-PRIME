...
class ManifestoController extends Controller
{
    ...
    {
        $locais = Localizacao::where('empresa_id', $request->empresa_id)->where('status', 1)
        ->get();
        return view('manifesto.nova_consulta', compact('locais'));
    }
    ...
    private function renderizaXml($xml){
        ...
        foreach ($xml->NFe->infNFe->det as $item) {
            ...
            ? $local
            : json_encode(collect($produto->locais)->pluck('localizacao_id')->toArray());
