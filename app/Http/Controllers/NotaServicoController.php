...
class NotaServicoController extends Controller
{
    ...
    public function create(){
        ...
        $nbs = Nbs::all();
        return view('nota_servico.create', compact('novoNumeroNFse', 'nbs'));
    }
    ...
    public function edit($id){
        ...
        $nbs = Nbs::all();
        return view('nota_servico.edit', compact('item', 'nbs'));
    }
        ...
        try{
            DB::transaction(function () use ($request) {
                ...
                ItemNotaServico::create([
                    ...
                    'codigo_nbs' => $request->codigo_nbs ?? null,
...
public function imprimir($id){
    ...
    if($item->estado == 'cancelado'){
        ...
        if(isset($resp->pdf)){
            ...
            echo "Não foi possível imprimir essa NFSe cancelada";
            die;
        }
        ...
        if(file_exists(public_path('nfse_pdf/').$item->chave.".pdf")){
            ...
            return response($pdf)
            ->header('Content-Type', 'application/pdf');
        }elseif($item->url_pdf_nfse){
            return redirect($item->url_pdf_nfse);
        }elseif($item->url_pdf_rps){
...
public function preview($id){
    ...
    try {
        ...
        if($servico->codigo_nbs){
            $payload['servico']['itens'][0]['codigo_nbs'] = preg_replace('/[^0-9]/', '', $servico->codigo_nbs);
            $payload['servico']['codigo_nbs'] = preg_replace('/[^0-9]/', '', $servico->codigo_nbs);
        }
        // dd($payload);
