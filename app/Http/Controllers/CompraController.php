...
class CompraController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        ->when(!empty($chave), function ($query) use ($chave) {
            return $query->where(function ($q) use ($chave) {
                $q->where('chave', 'like', "%{$chave}%")
                ->orWhere('chave_importada', 'like', "%{$chave}%");
            });
        })
    ...
    public function storeXml(Request $request)
    {
        if ($request->hasFile('file')) {
            ...
            foreach ($xml->NFe->infNFe->det as $item) {
                ...
                ? $local
                : json_encode(
                    optional($produto->locais)->pluck('localizacao_id')->toArray()
                );
    ...
    public function finishXml(Request $request)
    {
        ...
        try {
            $nfe = DB::transaction(function () use ($request) {
                ...
                if(isset($request->compra_selecionada) && $request->compra_selecionada){
                    $nfe = Nfe::findOrFail($request->compra_selecionada);
                    $nfe->estado_compra = 'finalizado';
                ...
                for ($i = 0; $i < sizeof($request->cProd); $i++) {
                    ...
                    if ($request->cadastrar_produto[$i] == 1) {
                        ...
                        //valor de venda
                        if($configGeral != null && $configGeral->atualizar_valor_venda_importacao == 1){
                    ...
                    if ($product->gerenciar_estoque) {
                        $this->util->incrementaEstoque($product->id, $quantidade, null, $local_id);
                        $tipo = 'incremento';
...
public function downloadXmlImportado($id)
{
    ...
    $chave = $item->chave_importada ?: $item->chave;

    $arquivos = [
        public_path('xml_entrada/' . $chave . '.xml'),
        public_path('xml_dfe/' . $chave . '.xml'),
    ];
    ...
    session()->flash("flash_error", "Arquivo XML não encontrado ou vazio");
    return redirect()->back();
}
...
public function rastro(Request $request){
    ...
    ->whereHas('item.nfe', function ($q) use ($empresaId) {
