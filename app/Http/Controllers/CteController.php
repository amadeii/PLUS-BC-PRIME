...
class CteController extends Controller
{
    ...
    public function duplicar($id)
    {
        ...
        $tiposMedida = Cte::tiposMedida();
        $cidades = Cidade::all();
        return view('cte.duplicar', compact('item', 'naturezas', 'clientes', 'veiculos', 'unidadesMedida', 'tiposMedida', 'cidades'));
    }
    ...
    public function imprimir($id)
    {
        ...
        $path = public_path('xml_cte/') . $item->chave . '.xml';

        if (file_exists($path)) {

            $xml = file_get_contents($path);

            $dacte = new Dacte($xml, $item->estado);

            if ($empresa->logo) {
                $logo = 'data://text/plain;base64,' . base64_encode(
                    file_get_contents(public_path('/uploads/logos/' . $empresa->logo))
                );

                $dacte->logoParameters($logo, 'L');
            }
