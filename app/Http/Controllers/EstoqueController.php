...
class EstoqueController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $totalProdutos = Produto::where('empresa_id', $request->empresa_id)
        ->where('status', 1)
        ->count();
    ...
    public function categoria(Request $request){
        ...
        ->select(
            ...
            DB::raw('
                SUM(
                estoques.quantidade *
                COALESCE(produto_variacaos.valor, produtos.valor_unitario, 0)
                ) as valor_venda
                '),
            DB::raw('
                SUM(
                estoques.quantidade *
                COALESCE(produtos.valor_compra, 0)
                ) as valor_compra
                ')
        )
        ->get();
    ...
    public function estoqueData(Request $request)
    {
        $dataConsulta = $request->data_consulta ?? date('Y-m-d');

        $categorias = CategoriaProduto::where('empresa_id', request()->empresa_id)
        ->where('status', 1)
        ->get();

        $ultimaMovimentacao = MovimentacaoProduto::query()
        ->join('produtos', 'produtos.id', '=', 'movimentacao_produtos.produto_id')
        ->join(DB::raw('(
            select 
            mp.produto_id,
            mp.produto_variacao_id,
            max(mp.id) as ultimo_id
            from movimentacao_produtos mp
            inner join produtos p on p.id = mp.produto_id
            where p.empresa_id = '.request()->empresa_id.'
            and date(mp.created_at) <= "'.$dataConsulta.'"
            group by mp.produto_id, mp.produto_variacao_id
        ) as ultimos'), function($join){
            $join->on('movimentacao_produtos.id', '=', 'ultimos.ultimo_id');
        })
        ->select('movimentacao_produtos.*');

        $data = Produto::where('empresa_id', request()->empresa_id)
        ->with(['categoria'])
        ->leftJoinSub($ultimaMovimentacao, 'mov', function($join){
            $join->on('produtos.id', '=', 'mov.produto_id');
        })
        ->select(
            'produtos.*',
            DB::raw('coalesce(mov.estoque_atual, 0) as estoque_data')
        );

        if($request->produto){
            $data->where('produtos.nome', 'like', "%$request->produto%");
        }

        if($request->codigo_barras){
            $data->where('produtos.codigo_barras', 'like', "%$request->codigo_barras%");
        }

        if($request->categoria_id){
            $data->where('produtos.categoria_id', $request->categoria_id);
        }

        $data = $data->paginate(__itensPagina());

        $totalProdutos = $data->total();

        $totalCompra = $data->sum(function($item){
            return $item->estoque_data * $item->valor_compra;
        });

        $totalVenda = $data->sum(function($item){
            return $item->estoque_data * $item->valor_unitario;
        });

        return view('estoque.data', compact(
            'data',
            'categorias',
            'dataConsulta',
            'totalProdutos',
