...
class WoocommerceProdutoController extends Controller
{
    ...
    //     try{
    //         $wooProdutos = $woocommerceClient->get('products', [
    //             'per_page' => 20,
    //             'page' => 1,
    //             'orderby' => 'id',
    //             'order' => 'desc'
    ...
    //     }catch(\Exception $e){
    //         session()->flash("flash_error", "Erro ao buscar produtos: " . $e->getMessage());
    ...
    //         session()->flash("flash_success", "Nenhum produto para vincular!");
    //         return redirect()->route('woocommerce-produtos.index');
    //     }

    //     $empresa = Empresa::findOrFail($request->empresa_id);

    //     $listaCTSCSOSN = $empresa->tributacao == 'Regime Normal'
    //     ? Produto::listaCST()
    //     : Produto::listaCSOSN();
    ...
    public function importar(Request $request){
        ...
        try{
            ...
            do {
                ...
                $wooProdutos = array_merge($wooProdutos, $response);
                $page++;
        ...
        if(sizeof($produtosIsert) == 0){
            session()->flash("flash_success", "Produtos já estão sincronizados!");
            return redirect()->route('woocommerce-produtos.index');
        }
            ...
            for($i=0; $i<sizeof($request->produto_check); $i++){
                ...
                if($produto != null){
                    ...
                    $data = [
                        ...
                        'slug' => $produto->woocommerce_slug ?: Str::slug($produto->nome),
                        'status' => 'publish',
                    ...
                    $produto->woocommerce_id = $product->id;
                    $produto->woocommerce_slug = $product->slug ?? null;
                    $produto->woocommerce_link = $product->permalink ?? null;
                    $produto->woocommerce_valor = $product->price ?? $produto->valor_unitario;
                    $produto->woocommerce_type = $product->type ?? $type;
                    $produto->woocommerce_status = $product->status ?? 'publish';
                    $produto->woocommerce_descricao = $product->description ?? $produto->woocommerce_descricao;
                    $produto->woocommerce_stock_status = $product->stock_status ?? 'instock';
                    $produto->save();
