...
class ProdutoController extends Controller
{
    ...
    {   
        ...
        ->when($request->nome_campo && $request->valor_campo, function ($q) use ($request) {
            ...
            if (in_array($request->nome_campo, $camposPermitidos)) {

                $valor = $request->valor_campo;

                if (in_array($request->nome_campo, ['ncm', 'cest'])) {
                    $valor = preg_replace('/\D/', '', $valor);

                    return $q->whereRaw(
                        "REPLACE(REPLACE(REPLACE({$request->nome_campo}, '.', ''), '-', ''), '/', '') LIKE ?",
                        ["%{$valor}%"]
                    );
                }

                $camposNumericos = ['numero_sequencial'];

                if (in_array($request->nome_campo, $camposNumericos)) {
                    return $q->where($request->nome_campo, $valor);
                }

                return $q->where($request->nome_campo, 'LIKE', "%{$valor}%");
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $request->merge([
                ...
                'perc_red_bc_ibs' => $request->perc_red_bc_ibs ?? 0,
...
public function update(Request $request, $id)
{
    ...
    try {
        ...
        if($request->estoque_atual){
            $transacao = Estoque::where('produto_id', $item->id)
            ->orderBy('id', 'desc')
            ->first();
            $qtd = __convert_value_bd($request->estoque_atual);
            $tipo_transacao = 'alteracao_estoque';

            if($transacao == null){
                $tipo = 'incremento';

                $this->utilEstoque->incrementaEstoque($item->id, $qtd, null);
                $transacao = Estoque::where('produto_id', $item->id)
                ->orderBy('id', 'desc')
                ->first();
                $codigo_transacao = $transacao->id;
                ...
                $estoqueAtual = $transacao->quantidade;
                $codigo_transacao = $transacao->id;
                if($qtd > $estoqueAtual){
                    $diferenca = $qtd - $estoqueAtual;
                    $tipo = 'incremento';

                    $this->utilEstoque->incrementaEstoque($item->id, $diferenca, null);
                }else if($qtd < $estoqueAtual){
                    $diferenca = $estoqueAtual - $qtd;
                    $tipo = 'reducao';

                    $this->utilEstoque->reduzEstoque($item->id, $diferenca, null);
                }else{
                    $diferenca = 0;
                    $tipo = 'sem_alteracao';
                }
...
private function __validate(Request $request)
{
    $rules = [
        ...
        'codigo_interno' => [
            'nullable',
            Rule::unique('produtos', 'codigo_interno')->where(function ($q) use($request) {
    ...
    $messages = [
        ...
        'codigo_interno.unique' => 'Já existe um produto com este código interno nesta empresa!',
    ];
...
public function show($id){
    ...
    $auditorias = AuditoriaRegistro::where('entidade', 'Produto')
    ->where('registro_id', $item->id)
    ->with('usuario')
    ->orderBy('id', 'desc')
    ->get();
    return view('produtos.show', compact('item', 'data', 'produtoUnico', 'auditorias'));
}
...
public function etiqueta(Request $request, $id)
{
    ...
    $temCodigoProduto = !empty($item->codigo_barras);

    $temCodigoVariacao = $item->variacoes()
    ->whereNotNull('codigo_barras')
    ->where('codigo_barras', '!=', '')
    ->exists();

    if (!$temCodigoProduto && !$temCodigoVariacao) {
        session()->flash('flash_error', 'Produto sem código de barras definido no produto ou nas variações');
        return redirect()->route('produtos.index');
    }
...
public function etiquetaStore(Request $request, $id)
{
    ...
    $item = Produto::with(['empresa', 'variacoes'])->findOrFail($id);

    $generatorPNG = new \Picqer\Barcode\BarcodeGeneratorPNG();

    $adds = [];

    $codigosSelecionados = $request->codigos_selecionados ?? [];

    if (sizeof($codigosSelecionados) == 0) {
        session()->flash('flash_error', 'Selecione ao menos um código de barras para gerar etiqueta');
        return redirect()->back();
    }

    foreach ($codigosSelecionados as $codigoSelecionado) {

        $produto = null;
        $variacao = null;

        if (str_starts_with($codigoSelecionado, 'produto_')) {
            $produtoId = str_replace('produto_', '', $codigoSelecionado);
            $produto = Produto::find($produtoId);
        }

        if (str_starts_with($codigoSelecionado, 'variacao_')) {
            $variacaoId = str_replace('variacao_', '', $codigoSelecionado);
            $variacao = ProdutoVariacao::with('produto')->find($variacaoId);
        ...
        $codigoBarras = $variacao ? $variacao->codigo_barras : $produto->codigo_barras;
        ...
        if (strlen($codigoBarras) == 12) {
            $bar_code = $generatorPNG->getBarcode($codigoBarras, $generatorPNG::TYPE_CODE_128);
        } else {
            $bar_code = $generatorPNG->getBarcode($codigoBarras, $generatorPNG::TYPE_EAN_13);
        }
        ...
        $adds[] = [
            ...
            'valor' => $variacao ? $variacao->valor : $produto->valor_unitario,
            'unidade' => $produto->unidade,
            'valor_atacado' => $produto->valor_atacado,
            'referencia' => $variacao?->referencia ?: $produto->referencia,
    ...
    if (sizeof($adds) == 0) {
        session()->flash('flash_error', 'Nenhum código de barras válido encontrado');
        return redirect()->back();
    }
...
//                 $codigoAdd = $prod->codigo_barras;

//                 $randAdd = rand(10000,99999);

//                 // $bar_code = $generatorPNG->getBarcode($codigoAdd, $generatorPNG::TYPE_EAN_13);
//                 // dd($codigoAdd);
//                 if (strlen($codigoAdd) == 12) {
//                     $bar_code = $generatorPNG->getBarcode($codigoAdd, $generatorPNG::TYPE_CODE_128);
//                 } else {
//                     $bar_code = $generatorPNG->getBarcode($codigoAdd, $generatorPNG::TYPE_EAN_13);
//                 }

//                 file_put_contents(public_path("barcode")."/$randAdd.png", $bar_code);
//                 $adds[] = [
//                     'nome_empresa' => $request->nome_empresa ? true : false,
//                     'nome_produto' => $request->nome_produto ? true : false,
//                     'valor_produto' => $request->valor_produto ? true : false,
...
//     if (strlen($codigo) == 12) {
//         $bar_code = $generatorPNG->getBarcode($codigo, $generatorPNG::TYPE_CODE_128);
//     } else {
//         $bar_code = $generatorPNG->getBarcode($codigo, $generatorPNG::TYPE_EAN_13);
//     }
...
public function reajuste(Request $request){
    ...
    if($nome || $categoria_id || $cst_csosn || $marca_id || $local_id || $pendentes || $data_cadastro){
        $data = Produto::where('empresa_id', $request->empresa_id)
        ->select('produtos.*')
        ...
        ->when(!empty($data_cadastro), function ($query) use ($data_cadastro) {
            $query->whereDate('produtos.created_at', $data_cadastro);
        })
...
public function reajusteUpdate(Request $request){
    try{
        for($i=0; $i<sizeof($request->produto_id); $i++){
            ...
            $item->categoria_id = $request->categoria[$i];
...
public function ibpt(Request $request)
{
    ...
    $ibptService = new IbptService(
        $empresa->token_ibpt,
        preg_replace('/[^0-9]/', '', $empresa->cpf_cnpj)
    );
    ...
    foreach ($produtos as $p) {
        ...
        try {
            ...
            if (!$resp || empty($resp->Codigo)) {
                continue;
            }

            ProdutoIbpt::updateOrCreate(
                ['produto_id' => $p->id],
                [
                    'codigo' => $resp->Codigo,
                    'uf' => $resp->UF,
            ...
            \Log::error("Erro IBPT Produto {$p->id}: " . $e->getMessage());

        }
    }

    $msg = "Total de produtos: $totalDeProdutos | Atualizados IBPT: $contIbpt";
