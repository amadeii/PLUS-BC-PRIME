...
class ContadorAdminNFCeController extends Controller
{
    ...
    public function nfce(Request $request)
    {
        ...
        $query = Nfce::where('empresa_id', $empresaSelecionada)
        ...
        $dir = $estado === 'cancelado' ? 'xml_nfce_cancelada/' : 'xml_nfce/';

        $chavesComXml = (clone $query)->get()
        ->filter(fn($item) => !empty($item->chave) && file_exists(public_path($dir . $item->chave . '.xml')))
        ->pluck('chave')
        ->toArray();

        $data = (clone $query)
        ->whereIn('chave', $chavesComXml)
        ->orderBy('created_at', 'desc')
        ->paginate(__itensPagina())
        ->appends($request->query());

        $contXml = count($chavesComXml);
