...
class MdfeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $encerrado = $request->get('encerrado');
        ...
        ->when($encerrado !== null && $encerrado !== '', function ($query) use ($encerrado) {
            return $query->where('encerrado', $encerrado);
        })
        ->when($importado !== null && $importado !== '', function ($query) use ($importado) {
            return $query->where('importado', $importado);
        })
    ...
    public function create()
    {
        ...
        $motoristas = Motorista::where('empresa_id', request()->empresa_id)
        ->orderBy('nome')->get();
        return view('mdfe.create', compact('veiculos', 'cidades', 'numeroMDFe', 'motoristas'));
    }
            ...
            DB::transaction(function () use ($request) {
                ...
                for ($i = 0; $i < sizeof($request->valor_componente); $i++) {
                    if ($request->valor_componente[$i] != null) {
                        ComponenteMdfe::create([
                            ...
                            'tipo' => $request->tipo_componente[$i] ?? '01',
                ...
                for ($i = 0; $i < sizeof($request->tp_und_transp_row); $i++) {
                    $info = InfoDescarga::create([
                        'mdfe_id' => $mdfe->id,
                        'tp_unid_transp' => $request->tp_und_transp_row[$i] ?? 1,
...
return redirect()->route('mdfe.index');
    ...
    public function naoEncerrados()
    {
        ...
        if ($config->arquivo == null) {
            session()->flash("flash_error", "Configure o certificado!");
        ...
        if (!isset($resultados['cStat'])) {
            session()->flash("flash_error", "Não foi possível consultar MDF-e não encerrados.");
            return redirect()->back();
        }

        // if ($resultados['cStat'] != '111' && $resultados['cStat'] != '112') {
        //     session()->flash("flash_error", $resultados['xMotivo'] ?? 'Erro ao consultar MDF-e não encerrados.');
        //     return redirect()->back();
        // }
    ...
    public function encerrar(Request $request)
    {
        try {
            ...
            if (!$config) {
                session()->flash("flash_error", "Empresa não encontrada.");
                return redirect()->back();
            }
            ...
            if (!$mdfe) {
                session()->flash("flash_error", "MDF-e não encontrada.");
                return redirect()->back();
            }

            if ($mdfe->encerrado) {
                session()->flash("flash_warning", "Esta MDF-e já está encerrada.");
                return redirect()->back();
            }

            $cnpjEmpresa = preg_replace('/[^0-9]/', '', $config->cpf_cnpj);
            $cnpjChave = substr($mdfe->chave, 6, 14);

            if ($cnpjEmpresa != $cnpjChave) {
                session()->flash("flash_error", "Não foi possível encerrar: o CNPJ da empresa atual não é o emissor desta MDF-e. MDF-e importada deve ser encerrada com o certificado do emitente original.");
                return redirect()->back();
            }
            ...
            $resp = $mdfe_service->encerrar($mdfe->chave, $mdfe->protocolo);

            $cStat = $resp->infEvento->cStat ?? null;
            $xMotivo = $resp->infEvento->xMotivo ?? 'Retorno não identificado da SEFAZ.';

            if (!in_array($cStat, [135, 136])) {
                session()->flash("flash_error", "Erro ao encerrar MDF-e: {$cStat} - {$xMotivo}");
                return redirect()->back();
            }

            $mdfe->encerrado = true;
            $mdfe->estado_emissao = 'encerrado';
            $mdfe->save();

            session()->flash("flash_success", $xMotivo);

        } catch (\Exception $e) {
            session()->flash("flash_error", "Erro ao encerrar MDF-e: " . $e->getMessage());
        }

        return redirect()->back();
    }

    public function importarXmlEncerrar(Request $request)
    {
        try {
            ...
            if ($request->hasFile('xml')) {
                ...
                $xmlObj->registerXPathNamespace('n', 'http://www.portalfiscal.inf.br/mdfe');

                $chave = (string)$xmlObj->xpath('//n:infMDFe')[0]['Id'];
                $chave = str_replace('MDFe', '', $chave);

                $protocolo = (string)$xmlObj->xpath('//n:protMDFe/n:infProt/n:nProt')[0];

                return redirect()->route('mdfe.encerrar', [
                    'chave' => $chave,
                    'protocolo' => $protocolo
                ]);
            }
    ...
    public function createByVendas($ids)
    {
        ...
        $gruposPagamento = GrupoPagamentoPadrao::where('empresa_id', request()->empresa_id)
        ->orderBy('nome_pagador')->get();

        return view('mdfe.importarNfe.create', compact('numeroMDFe', 'veiculos', 'cidades', 'nfe', 'empresa', 'motoristas', 'gruposPagamento'));
    }
    ...
    private function tratarDados($ids){
        ...
        foreach($ids as $key => $i){
            ...
            if($key == 0){
                $item['produto_predominante'] = $nfe->itens[0]->produto->nome;
                $item['produto_codigo_barras'] = $nfe->itens[0]->produto->codigo_barras;
                $item['produto_ncm'] = $nfe->itens[0]->produto->ncm;

            }

            if($nfe->cliente){

                $cep = preg_replace('/\D/', '', $nfe->cliente->cep);
                $item['cep_descarrega'] = substr($cep, 0, 5) . '-' . substr($cep, 5, 3);
                ...
                $cep = preg_replace('/\D/', '', $nfe->fornecedor->cep);
                $item['cep_descarrega'] = substr($cep, 0, 5) . '-' . substr($cep, 5, 3);
            ...
            if($xml != null){
                ...
                if(isset($veiculo['placa'])){
                    ...
                    $veiculo = Veiculo::where('empresa_id', request()->empresa_id)
                    ->where('placa', $veiculo['placa'])->first();
                    if($veiculo != null){
                        $item['veiculo_id'] = $veiculo->id;
                    }
                }
            }
