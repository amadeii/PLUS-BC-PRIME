...
class RelatorioController extends Controller
{
    public function index()
    {
        ...
        $categoriasContaReceber = CategoriaConta::where('empresa_id', request()->empresa_id)
        ->where('status', 1)->where('tipo', 'receber')->get();
        $categoriasContaPagar = CategoriaConta::where('empresa_id', request()->empresa_id)
        ...
        return view('relatorios.index', compact('funcionarios', 'marcas', 'categorias', 'tiposDespesaFrete', 
            'temPlanoConta', 'categoriasContaReceber', 'categoriasContaPagar', 'usuarios'));
    }
    ...
    public function conta_pagar(Request $request)
    {
        ...
        ->when($categoria_id, function ($query) use ($categoria_id) {
            return $query->where('categoria_conta_id', $categoria_id);
        })
        ->when($local_id, function ($query) use ($local_id) {
            return $query->where('local_id', $local_id);
        })
    ...
    public function vendas(Request $request)
    {
        ...
        foreach($data as $item){
            ...
            if(isset($item['fatura']) && count($item['fatura']) > 0){
                ...
                foreach($item['fatura'] as $f){
                    ...
                    if(!isset($totaisPagamento[$tipo])){
                        $totaisPagamento[$tipo] = 0;
                    }

                    $totaisPagamento[$tipo] += (float)$f['valor'];
                }
                ...
                $totaisPagamento[$tipo] += (float)$item['total'];
            }
        }
        ...
        arsort($totaisPagamento);
        $p = view('relatorios/vendas', compact('data', 'tipo', 'totaisPagamento'))
    ...
    public function estoque(Request $request)
    {
        ...
        if ($local_id) {
            $query->where('estoques.local_id', $local_id);
        }

        if ($categoria_id) {
            $query->where(function ($q) use ($categoria_id) {
                $q->where('produtos.categoria_id', $categoria_id)
                ->orWhere('produtos.sub_categoria_id', $categoria_id);
            });
        }

        if ($start_date) {
            $query->whereDate('produtos.created_at', '>=', $start_date);
        }

        if ($end_date) {
            $query->whereDate('produtos.created_at', '<=', $end_date);
        }

        $rows = $query->select([
            'produtos.numero_sequencial as codigo',
            DB::raw("
                CASE 
                WHEN produto_variacaos.id IS NULL 
                THEN produtos.nome
                ELSE CONCAT(produtos.nome, ' ', produto_variacaos.descricao)
                END as produto
                "),
            DB::raw('COALESCE(estoques.quantidade, 0) as quantidade'),
            'produtos.estoque_minimo',
            DB::raw('COALESCE(produtos.valor_compra, 0) as valor_compra'),
            DB::raw('COALESCE(produto_variacaos.valor, produtos.valor_unitario, 0) as valor_venda'),
            DB::raw("COALESCE(categoria_produtos.nome, '--') as categoria"),
        ...
        foreach ($rows as $r) {
            ...
            if ($estoque_minimo == 1) {
                if ($r->quantidade > $r->estoque_minimo) {
                    continue;
                }
            }
            ...
            $data[] = [
                ...
                'data_cadastro' => __data_pt($r->data_cadastro)
            ];
        }
        ...
        if ($esportar_excel == -1) {
            ...
            $p = view('relatorios/estoque', compact(
                'data',
                'start_date',
                'end_date',
                'estoque_minimo',
            ...
            return $domPdf->stream("Relatório de estoque.pdf", ["Attachment" => false]);
    ...
    public function tiposDePagamento(Request $request)
    {
        ...
        foreach ($nves as $n) {
            foreach ($n->fatura as $f) {
                if (isset($data[$f->tipo_pagamento])) {
                    $data[$f->tipo_pagamento] += (float) $f->valor;
                }
            }
        }
        ...
        $domPdf->setPaper("A4", "landscape");
        $domPdf->render();
        $domPdf->stream("Relatório de Tipos de Pagamento.pdf", ["Attachment" => false]);
    }
    ...
    public function movimentacoes(Request $request){
        ...
        ->when(!empty($usuario_id), function ($query) use ($usuario_id) {
            return $query->where('movimentacao_produtos.user_id', $usuario_id);
        })
        ->when(!empty($produto_id), function ($query) use ($produto_id) {
            return $query->where('movimentacao_produtos.produto_id', $produto_id);
        })
        ->join('produtos', 'produtos.id', '=', 'movimentacao_produtos.produto_id')
        ->where('produtos.empresa_id', $request->empresa_id)
        ->get();

        $view = view('relatorios.movimentacoes', [
            'data' => $data,
            'start_date' => $request->start_date,
            'end_date' => $request->end_date,
        ...
        $domPdf->setPaper('A4', 'landscape');
        $domPdf->render();
        $domPdf->stream('Relatorio_movimentações.pdf', ['Attachment' => false]);
