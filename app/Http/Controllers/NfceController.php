...
class NfceController extends Controller
{
    ...
    public function duplicar($id)
    {
        ...
        return view('nfce.duplicar', compact('item', 'cidades', 'naturezas', 'caixa'));
    }
...
public function imprimirTef($id){
    $venda = Nfce::findOrFail($id);
    __validaObjetoEmpresa($venda);
    $tefLog = $venda->tefLog;

    $configuracaoTef = ScopeConfig::where('empresa_id', $venda->empresa_id)
    ->where('usuario_id', get_id_user())
    ->where('habilitado', 1)
    ->first();

    if($configuracaoTef != null && $configuracaoTef->habilitado == 1){

        $tef_raw = $tefLog->tef_raw;

        if (is_string($tef_raw)) {
            $tef_raw = json_decode($tef_raw, true);
        }

        $cliente = collect($tef_raw['impressoCliente'] ?? [])
        ->filter()
        ->map(fn($linha) => (string) $linha)
        ->values()
        ->toArray();

        $loja = collect($tef_raw['impressoLoja'] ?? [])
        ->filter()
        ->map(fn($linha) => (string) $linha)
        ->values()
        ->toArray();
        ...
        $html = view('front_box.comprovante_80mm', $dados)->render();
    }else{

        $dados = [
            'cliente' => $this->normalizarComprovante($tefLog->comprovantes['cliente'] ?? []),
            'loja'    => $this->normalizarComprovante($tefLog->comprovantes['loja'] ?? []),
        ];
        $html = view('front_box.comprovante_80mm', $dados)->render();
    ...
    $dompdf->setPaper([0, 0, 226.77, 200], 'portrait');

    $dompdf->render();

    return $dompdf->stream('comprovante-tef.pdf', [
        'Attachment' => false
    ]);

}

private function normalizarComprovante($comprovante)
{
    if (is_string($comprovante)) {
        return preg_split("/\r\n|\n|\r/", $comprovante);
    }

    return is_array($comprovante) ? $comprovante : [];
}
...
public function destroy($id)
{
    ...
    try {
        ...
        $cliente = $item->cliente?->info ?? 'Consumidor não informado';
        $descricaoLog = $cliente . ' R$ ' . __moeda($item->total);
...
public function storeEstado(Request $request, $id)
{
    ...
    try {
        ...
        $descricaoLog = "Estado alterado para $item->estado, NFCe #$item->numero_sequencial";
        __createLog($request->empresa_id, 'NFCe', 'corrigir', $descricaoLog);
        $item->save();
