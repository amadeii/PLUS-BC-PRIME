...
class NfeController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if (preg_match('/Android|iPhone|iPad|iPod|Mobile/i', $ua) && file_exists(public_path('style_pdv_mobo.css')) && $isOrcamento == 0) {
        ...
        return view('nfe.create', 
            compact('transportadoras', 'cidades', 'naturezas', 'numeroNfe', 'empresa', 'caixa', 
                'isOrcamento', 'naturezaPadrao', 'config', 'funcionarios', 'cadastroSimplificado', 'vendedor')
        );
    }
    ...
    public function edit($id)
    {
        ...
        $cadastroSimplificado = $config && $config->cadastro_simplificado_cliente;
        return view('nfe.edit', compact('item', 'transportadoras', 'cidades', 'naturezas', 'caixa', 'funcionarios', 'config', 'cadastroSimplificado'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $nfe = DB::transaction(function () use ($request) {
                ...
                if(!$caixa){
                    session()->flash("flash_warning", "Caixa fechado, abra para salvar a venda!");
                    return redirect()->back();
                }
                ...
                if ($request->ordem_producao_id) {
                    $ordem = OrdemProducao::findOrFail($request->ordem_producao_id);
                    $ordem->nfe_id = $nfe->id;
                    $ordem->save();
                }
...
public function storeEstado(Request $request, $id)
{
    ...
    try {
        ...
        $item->save();
        $descricaoLog = "Estado alterado para $item->estado, Venda #$item->numero_sequencial";
        __createLog($request->empresa_id, 'Venda', 'corrigir', $descricaoLog);
...
public function imprimirDivergencias($id){
    $compra = Nfe::with([
        'fornecedor',
        'divergencias'
    ])->findOrFail($id);

    $divergenciasItens = $compra->divergencias->where('tipo','item');
    $divergenciasFaturas = $compra->divergencias->where('tipo','fatura');
    $divergenciaTotal = $compra->divergencias->firstWhere('tipo','total');

    $config = config('app'); 

    $p = view('compras.imprimir_divergencias', compact(
        'compra',
        'divergenciasItens',
        'divergenciasFaturas',
        'divergenciaTotal',
    ...
    header("Content-Disposition: ; filename=divergencias.pdf");

    $domPdf->stream(
        "Divergencias_Compra_".$compra->id.".pdf",
        ["Attachment" => false]
    );
}
...
private function preparaXmls($destino)
{
    $files = new \RecursiveIteratorIterator(
        new \RecursiveDirectoryIterator($destino)
    );
    ...
    foreach ($files as $file) {
        ...
        try {
            ...
            // XML pode vir como <nfeProc><NFe> ou direto <NFe>
            if (isset($xml->NFe)) {
                $nfe = $xml->NFe;
            ...
            $cliente = $this->getClientePeloNfe($nfe);
            $produtos = $this->getProdutosPeloNfe($nfe);
            $fatura = $this->getFaturaPeloNfe($nfe);
            ...
            if ($produtos != null && count($produtos) > 0) {
                ...
                $item = [
                    ...
                    'chave' => substr((string)$infNFe->attributes()->Id, 3, 44),
                    'valor_total' => (float)$infNFe->total->ICMSTot->vProd,
                    'numero_nfe' => (int)$infNFe->ide->nNF,
                    'desconto' => (float)$infNFe->total->ICMSTot->vDesc,
                    'cliente' => $cliente,
                    'produtos' => $produtos,
                    'fatura' => $fatura,
                    'file' => $path,
                    'natureza' => (string)$infNFe->ide->natOp,
                    'observacao' => isset($infNFe->infAdic->infCpl) ? (string)$infNFe->infAdic->infCpl : '',
                    'tipo_pagamento' => isset($infNFe->pag->detPag->tPag) ? (string)$infNFe->pag->detPag->tPag : '',
                    'finNFe' => (string)$infNFe->ide->finNFe,
            ...
            \Log::error('Erro ao preparar XML importado', [
                'arquivo' => $path,
                'erro' => $e->getMessage(),
                'linha' => $e->getLine()
            ]);
...
private function getClientePeloNfe($nfe)
{
    $xml = new \stdClass();
    $xml->NFe = $nfe;
    return $this->getCliente($xml);
}

private function getProdutosPeloNfe($nfe)
{
    $xml = new \stdClass();
    $xml->NFe = $nfe;

    return $this->getProdutos($xml);
}

private function getFaturaPeloNfe($nfe)
{
    $xml = new \stdClass();
    $xml->NFe = $nfe;

    return $this->getFatura($xml);
}
...
private function getCliente($xml){
    ...
    if (isset($xml->NFe->infNFe->dest->enderDest->cMun)) {
        $cidade = Cidade::where('codigo', (string)$xml->NFe->infNFe->dest->enderDest->cMun)->first();
    }

    $enderDest = $xml->NFe->infNFe->dest->enderDest ?? null;

    $dadosCliente = [
        'cpf_cnpj' => isset($xml->NFe->infNFe->dest->CNPJ) ? (string)$xml->NFe->infNFe->dest->CNPJ : (string)($xml->NFe->infNFe->dest->CPF ?? ''),
        'razao_social' => (string)($xml->NFe->infNFe->dest->xNome ?? 'Cliente exterior'),
        'nome_fantasia' => (string)($xml->NFe->infNFe->dest->xFant ?? ''),
        'rua' => $enderDest ? (string)($enderDest->xLgr ?? '') : '',
        'numero' => $enderDest ? (string)($enderDest->nro ?? '') : '',
        'bairro' => $enderDest ? (string)($enderDest->xBairro ?? '') : '',
        'cep' => $enderDest ? (string)($enderDest->CEP ?? '') : '',
        'telefone' => $enderDest ? (string)($enderDest->fone ?? '') : '',
        'ie' => isset($xml->NFe->infNFe->dest->IE) ? (string)$xml->NFe->infNFe->dest->IE : '',
        'cidade_id' => $cidade ? $cidade->id : 1,
        'cidade_info' => $cidade ? "$cidade->nome ($cidade->uf)" : 'Exterior',
        'consumidor_final' => isset($xml->NFe->infNFe->dest->IE) && (string)$xml->NFe->infNFe->dest->IE ? 1 : 0,
        'status' => 1,
        'contribuinte' => 1,
        'empresa_id' => request()->empresa_id
    ];
...
private function getProdutos($xml){
    ...
    try{
        ...
        foreach($xml->NFe->infNFe->det as $item) {
            ...
            $ibscbs = $this->extrairIBSCBS($item);
            $itemData = array_merge($itemData, $ibscbs);
...
private function extrairIBSCBS($item)
{
    $imposto = $item->imposto ?? null;
    $ibscbs = $imposto->IBSCBS ?? null;

    return [
        'cst_ibscbs' => (string)($ibscbs->CST ?? ''),
        'cclass_trib' => (string)($ibscbs->cClassTrib ?? ''),
        'perc_ibs_uf' => isset($ibscbs->pIBSUF) ? (float)$ibscbs->pIBSUF : 0,
        'perc_ibs_mun' => isset($ibscbs->pIBSMun) ? (float)$ibscbs->pIBSMun : 0,
        'perc_cbs'     => isset($ibscbs->pCBS) ? (float)$ibscbs->pCBS : 0,
    ];
}
...
public function unificar(Request $request)
{
    ...
    if (count($ids) !== count($tipos)) {
        return response()->json([
            'error' => 'IDs e Tipos incompatíveis'
        ], 400);
    }
    ...
    /*
    ======================================
    BUSCAR REGISTROS NAS DUAS TABELAS
    ======================================
    */
    ...
    foreach ($ids as $index => $id) {
        ...
        if ($tipo === 'nfe') {
            $registro = Nfe::find($id);
        } elseif ($tipo === 'nfce') {
            $registro = Nfce::find($id);
        } else {
            return response()->json([
                'error' => 'Tipo inválido'
            ], 400);
        ...
        if (!$registro) {
            return response()->json([
                'error' => "Registro ID {$id} não encontrado"
            ], 404);
        }
    ...
    */
    ...
    $empresaId = $registros->first()->empresa_id;
    $clienteId = $registros->first()->cliente_id;

    if ($registros->where('estado', 'aprovado')->count()) {
        return response()->json([
            'error' => 'Não pode unificar notas aprovadas'
        ], 400);
    }
    ...
    try {
        ...
        /*
        ======================================
        MONTAR OBSERVAÇÃO PROFISSIONAL
        ======================================
        */
        ...
        foreach ($registros as $r) {
            ...
            $numero = $r->numero_sequencial ?? $r->numero ?? $r->id;
            $data = \Carbon\Carbon::parse($r->created_at)->format('d/m/Y');

            $descricaoNotas[] = "Nº {$numero} de {$data}";
        }

        $observacao = "Venda gerada pela unificação das vendas: "
        . implode(', ', $descricaoNotas) . ".";
        ...
        */
        ...
        $nova = Nfe::create([
            ...
            'natureza_id' => $registros[0]->natureza_id,
            'local_id' => $registros[0]->local_id,
            'emissor_nome' => $config->nome,
            'emissor_cpf_cnpj' => $config->cpf_cnpj,
            'ambiente' => $config->ambiente,
        ...
        */
        ...
        foreach ($registros as $registro) {
            ...
            foreach ($registro->itens as $i) {
                ItemProducao::where('item_id', $i->id)->delete();
                ItemDimensaoNfe::where('item_nfe_id', $i->id)->delete();
                RastroXml::where('item_nfe_id', $i->id)->delete();
            }

            $comissao = ComissaoVenda::where('empresa_id', $registro->empresa_id)
            ->where('nfe_id', $registro->id)->first();

            if($comissao){
                $comissao->delete();
            }

            $garantia = Garantia::where('empresa_id', $registro->empresa_id)
            ->where('nfe_id', $registro->id)->first();
            ...
            ContaPagar::where('nfe_id', $registro->id)->delete();
            ContaReceber::where('nfe_id', $registro->id)->delete();
            MovimentacaoProduto::where('codigo_transacao', $registro->id)->delete();

            $registro->itens()->delete();
            $registro->fatura()->delete();
            ProdutoUnico::where('nfe_id', $id)->delete();
            $registro->delete();
        }

        if ($request->filled('faturas') && is_array($request->faturas)) {

            $somaFaturas = 0;

            foreach ($request->faturas as $fatura) {
                if (
                    empty($fatura['tipo_pagamento']) ||
                    !isset($fatura['valor'])
                ) {
                ...
                FaturaNfe::create([
                    ...
                    'data_vencimento' => $fatura['data_vencimento'] ?? date('Y-m-d'),
                    'valor'           => $valor
                ]);

                $dataVencimento = !empty($fatura['data_vencimento'])
                ? \Carbon\Carbon::parse($fatura['data_vencimento'])
                : $hoje;

                $hoje = \Carbon\Carbon::today();

                if ($dataVencimento->greaterThan($hoje)) {
                    ContaReceber::create([
                        ...
                        'status' => 0,
                        'local_id' => $registros[0]->local_id,
                        'descricao' => 'Parcela referente à NFe '.$nova->numero_sequencial
                    ]);
                }
            }
