...
class ImportadorController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $destino = storage_path('app/importador');
        ...
        $zip = new \ZipArchive();
        $arquivoZip = $request->file('file')->getRealPath();

        if ($zip->open($arquivoZip) !== TRUE) {
            return back()->with('flash_error', 'Erro ao abrir arquivo ZIP.');
        }
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if (!preg_match('/\.(xlsx|xml)$/i', $nome)) {
                continue;
            }

            $zip->extractTo($destino, [$nome]);
        }
        ...
        try {
            ...
            return back()->with('flash_error', 'Erro na importação: ' . $e->getMessage());
        }

        return redirect()->route('home')
        ->with('flash_success', 'Dados importados com sucesso!');
    }
            ...
            foreach ($row as $key => $r) {
                if($key > 0){
                    ...
                    ->where(function ($q) use ($r) {
                        if (!empty($r[0])) {
                            $q->where('_id_import', $r[0]);
                        }

                        // if (!empty($r[5])) {
                        //     $q->orWhere('codigo_barras', $r[5]);
                        // }
                    ...
                    if ($produto) {
                        ...
                        $produto->update($objeto);
                    } else {
                        $produto = Produto::create($objeto);
                    }


                    ProdutoLocalizacao::firstOrCreate([
