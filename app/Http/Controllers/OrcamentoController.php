...
class OrcamentoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('orcamento.index', compact('data', 'funcionarios', 'ordemSeparacao', 'envioWppLink'));
    }
    ...
    public function imprimir80mm($id)
    {
        $item = Nfe::findOrFail($id);
        __validaObjetoEmpresa($item);
        ...
        return view('orcamento.imprimir_80mm', compact('config', 'item'));
    }
    ...
    public function gerarVenda($id)
    {
        ...
        foreach($data->itens as $i){
            $product = $i->produto;
            if ($product->gerenciar_estoque && $data->natureza->movimentar_estoque == 1) {
                $this->estoqueUtil->reduzEstoque($product->id, $i->quantidade, $i->variacao_id, $data->local_id);
            }
        }
        ...
        $planoContaId = null;
        if($data->natureza->planoConta){
            $planoContaId = $data->natureza->planoConta->id;
        }
        $cont = 1;
        foreach($data->fatura as $f){
            if (Carbon::parse($f->data_vencimento)->gt(Carbon::today())) {
                ContaReceber::create([
                    ...
                    'descricao' => "Parcela " . $cont . " de " . sizeof($data->fatura),
                    'referencia' => "Pedido " . $data->numero_sequencial,
    ...
    public function gerarVendaNfce($id)
    {
        ...
        foreach($item->itens as $i){
            ...
            foreach($item->itens as $i){
                $product = $i->produto;
                if ($product->gerenciar_estoque && $nfce->natureza->movimentar_estoque == 1) {
                    $this->estoqueUtil->reduzEstoque($product->id, $i->quantidade, $i->variacao_id, $nfce->local_id);
                }
            }
        ...
        foreach($item->fatura as $f){
            ...
            $dataFatura['nfce_id'] = $nfce->id;
            // dd($dataFatura);
            FaturaNfce::create($dataFatura);

            if (
                !empty($f->data_vencimento) &&
                Carbon::parse($f->data_vencimento)->startOfDay()->gt(now()->startOfDay())
            ) {
                ContaReceber::create([
                    ...
                    'descricao' => "Parcela " . $cont . " de " . sizeof($item->fatura),
                    'referencia' => "PDV " . $nfce->numero_sequencial,
                    'plano_conta_id' => $planoContaId
                ]);
                $cont++;
            }
    ...
    public function edit($id)
    {
        ...
        return view('nfe.edit', compact('item', 'transportadoras', 'cidades', 'naturezas', 'caixa', 'isOrcamento', 
            'funcionarios', 'cadastroSimplificado'));
    }

    public function ordemProducao($id){
        return redirect()->route('ordem-producao.create', ['orcamento_id='.$id]);
    }
}
