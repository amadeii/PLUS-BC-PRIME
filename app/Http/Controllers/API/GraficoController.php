...
class GraficoController extends Controller
{
    ...
    public function dadosCards(Request $request){
        ...
        ->when($periodo == 7, function ($query) {
            return $query->whereRaw('WEEK(data_vencimento) = ' . (date('W')-1))->whereYear('data_vencimento', date('Y'));
        })
        ->when($periodo == 30, function ($query) {
            return $query->whereMonth('data_vencimento', date('m'))->whereYear('data_vencimento', date('Y'));
        })
        ->when($periodo == 365, function ($query) {
        ...
        ->selectRaw('COUNT(*) as quantidade, SUM(total) as valor')
        ->first();

        $totalNfceAprovadas = Nfce::
        where('empresa_id', $empresa_id)
        ->where('estado', 'aprovado')
        ->when($periodo == 1, function ($query) {
            return $query->whereDate('created_at', date('Y-m-d'));
        ...
        $data = [
            ...
            'nfe_aprovadas' => $totalNfeAprovadas->quantidade ?? 0,
            'nfe_aprovadas_valor' => $totalNfeAprovadas->valor ?? 0,

            'nfce_aprovadas' => $totalNfceAprovadas->quantidade ?? 0,
            'nfce_aprovadas_valor' => $totalNfceAprovadas->valor ?? 0,
        ];
    ...
    public function graficoVendasMes(Request $request)
    {
        ...
        $nfe = Nfe::selectRaw('DAY(created_at) as dia, SUM(total) as total')
        ...
        $nfce = Nfce::selectRaw('DAY(created_at) as dia, SUM(total) as total')
        ->where('empresa_id', $request->empresa_id)
        ->where('estado', '!=', 'cancelado')
        ->whereMonth('created_at', $mes)
        ->whereYear('created_at', $ano)
        ...
        for ($i = 1; $i <= $diaHoje; $i++) {
            ...
            $totalNfe = (float)($nfe[$i] ?? 0);
            $totalNfce = (float)($nfce[$i] ?? 0);

            $data[] = [
                'dia' => str_pad($i, 2, '0', STR_PAD_LEFT) . "/$mes",
                'valor' => round($totalNfe + $totalNfce, 2)
            ];
        }
    ...
    public function graficoMesContador(Request $request)
    {
        ...
        $nfe = Nfe::selectRaw('DAY(created_at) as dia, COUNT(id) as total')
        ->where('empresa_id', $request->empresa_id)
        ->whereIn('estado', ['aprovado', 'cancelado'])
        ->whereMonth('created_at', $mes)
        ->whereYear('created_at', $ano)
        ...
        $nfce = Nfce::selectRaw('DAY(created_at) as dia, COUNT(id) as total')
        ->where('empresa_id', $request->empresa_id)
        ->whereIn('estado', ['aprovado', 'cancelado'])
        ->whereMonth('created_at', $mes)
        ->whereYear('created_at', $ano)
        ...
        for ($i = 1; $i <= $diaHoje; $i++) {
            $data[] = [
                'dia' => str_pad($i, 2, '0', STR_PAD_LEFT) . "/$mes",
                'valor' => (int) ($nfe[$i] ?? 0) + (int) ($nfce[$i] ?? 0)
            ];
        }
