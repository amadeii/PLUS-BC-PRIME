...
class CardapioController extends Controller
{
    ...
    public function categoria(Request $request, $id)
    {
        ...
        $categoria->produtosCardapio = Produto::where('empresa_id', $request->empresa_id)
        ->where('categoria_id', $categoria->id)
        ->where('status', 1)
        ->where('cardapio', 1)
        ->orderBy('nome')
        ->with(['pizzaValores', 'categoria'])
        ->get();
    ...
    public function storePedido(Request $request)
    {
        ...
        foreach($request->carrinho as $cartItem){
            ...
            if(isset($cartItem->remover_ingredientes) && $cartItem->remover_ingredientes != ''){
            ...
            $itemPedido = ItemPedido::create([
                ...
                'tamanho_id' => $cartItem->tamanho_id ?? null,
                'estado' => $estado,
                'impresso' => $impresso
            ]);
