...
class FrontBoxController extends Controller
{
    ...
    public function faturaCliente($cliente)
    {
        $faturas = FaturaCliente::where('cliente_id', $cliente)->get();
        ...
        $tipos = [
            ...
            '05' => 'Crédito Loja',
            '06' => 'Crediário',
            '07' => 'Cartão de Crédito TEF',
            '08' => 'Cartão de Débito TEF',
            '10' => 'Vale Alimentação',
            '11' => 'Vale Refeição',
            '12' => 'PIX TEF',
            '13' => 'Vale Presente',
        ...
        return response()->json([
            'tem_fatura' => true,
            'faturas' => $faturas->map(function ($f) use ($tipos) {
                return [
                    'id' => $f->id,
                    'tipo_pagamento' => $f->tipo_pagamento,
                    'tipo_pagamento_nome' => $tipos[$f->tipo_pagamento] ?? $f->tipo_pagamento,
                    'dias_vencimento' => (int)$f->dias_vencimento
                ];
            })->values()
        ]);
    }
    ...
    public function linhaProdutoVenda(Request $request)
    {
        try {
            ...
            if($product->combo && $product->lancar_produtos_combo){
                $html = '';

                foreach ($product->itensDoCombo as $itemCombo) {
                    $produtoFilho = $itemCombo->produtoDoCombo;

                    if (!$produtoFilho) {
                        continue;
                    }

                    $qtdItem = $qtd * ($itemCombo->quantidade ?? 1);
                    $valueUnitItem = $itemCombo->valor_compra;
                    $subTotalItem = $qtdItem * $valueUnitItem;

                    $html .= view('front_box.partials.row_frontBox', [
                        'product' => $produtoFilho,
                        'qtd' => $qtdItem,
                        'value_unit' => $valueUnitItem,
                        'sub_total' => $subTotalItem,
                        'key' => uniqid(),
                        'variacao_id' => null,
                        'variacao' => null,
                        'combo_nome' => $product->nome
                    ])->render();
                }
    ...
    public function linhaProdutoVendaAdd(Request $request)
    {
        ...
        if($product->unidade == 'KG'){
            return response("produto com kilo", 403);
        }
        $qtd = __convert_value_bd($request->qtd);
        try{
            $qtd = (float)$qtd+1;
    ...
    public function suspenderVenda4(Request $request)
    {
        try {
            ...
            return response()->json([
                'status' => false,
                'message' => 'Erro ao suspender venda.',
                'error' => $e->getMessage()
            ], 500);
        }
    }

    public function cancelarVendaSuspensa($id)
    {
        try {

            $venda = VendaSuspensa::findOrfail($id);
    ...
    public function continuarVendaSuspensa($id)
    {
        $venda = VendaSuspensa::with(['cliente', 'itens.produto'])
        ->where('id', $id)
        ->first();
        ...
        foreach ($venda->itens as $i) {
            $itens[] = [
                ...
                'img' => $i->produto->imgApp ?? null,
                'disponivel' => $i->produto->estoque ?? 0,
                'gerenciar_estoque' => $i->produto->gerenciar_estoque ?? 0,
            ];
        }
        ...
        return response()->json([
            'id' => $venda->id,
            'cliente_id' => $venda->cliente_id,
            'cliente_nome' => $venda->cliente->razao_social ?? $venda->cliente->nome ?? 'Consumidor',
    ...
    public function vendasSuspensasPdv4(Request $request)
    {
        $filtro = $request->filtro;

        $query = VendaSuspensa::with(['cliente', 'itens'])
        ...
        if ($filtro) {
            $query->where(function($q) use ($filtro){
                ...
                ->orWhereHas('cliente', function($sub) use ($filtro){
                  $sub->where('razao_social', 'like', "%{$filtro}%");
              });
            });
        } else {
            $query->limit(10);
        ...
        $data = $vendas->map(function($v){

            return [
                'id' => $v->id,
                'data' => $v->created_at ? $v->created_at->format('d/m/Y') : '--',
                'hora' => $v->created_at ? $v->created_at->format('H:i') : '--',
                ...
                'situacao' => 'Aguardando',
                'origem' => 'PDV',
                'itens' => $v->itens ? $v->itens->count() : 0,
                'desconto' => 'R$ ' . number_format($v->desconto ?? 0, 2, ',', '.'),

                'total' => 'R$ ' . number_format(
                    ($v->total + ($v->acrescimo ?? 0)) - ($v->desconto ?? 0),
                    2, ',', '.'
                ),
            ];
        });
    ...
    public function store(Request $request)
    {
        try {
            ...
            $nfce = DB::transaction(function () use ($request) {
                ...
                if(!$caixa){
                    return response()->json("Caixa fechado!", 401);
                }
                ...
                if(isset($request->venda_temporaria_id) && $request->venda_temporaria_id){
                    $vendaTemporaria = VendaTemporaria::find($request->venda_temporaria_id);
                    if($vendaTemporaria){
                        $vendaTemporaria->estado = 'finalizada';
                        $vendaTemporaria->total = $nfce->total;
                        $vendaTemporaria->venda_vinculada = $nfce->id;
                        $vendaTemporaria->save();
                    }
                }

                $tpPagTef = null;
                if(isset($request->tef_session_id) && $request->tef_session_id != null){
                    $tefLog = TefLog::where('empresa_id', $request->empresa_id)
                    ->where('tef_session_id', $request->tef_session_id)->first();

                    if($tefLog == null){
                        $tefLog = TefLog::where('empresa_id', $request->empresa_id)
                        ->where('tef_codigo_autorizacao', $request->tef_session_id)->first();
                    }

                    if($tefLog){
                        $tefLog->venda_id = $nfce->id;
                        $tefLog->save();

                        if($tefLog->tef_function_id == 2){
                            $tpPagTef = '04';
                        }elseif($tefLog->tef_function_id == 3){
                            $tpPagTef = '03';
                        }else{
                            $tpPagTef = '17';
                        }
                ...
                $planoContaId = null;
                if($nfce->natureza->planoConta){
                    $planoContaId = $nfce->natureza->planoConta->id;
                }
                    ...
                    if($pagamentos){
                        ...
                        foreach($pagamentos as $p){
                            ...
                            ContaReceber::create([
                                ...
                                'valor_recebido' => 0,
                                'status' => strtotime($vencimento) > strtotime($dataAtual) ? 0 : 1,
                                'descricao' => 'Venda PDV #' . $nfce->numero_sequencial . " Parcela " . $contParcela . " de " . sizeof($pagamentos),
                                'empresa_id' => $request->empresa_id,
                                'juros' => 0,
                                'multa' => 0,
                                'observacao' => '',
                                'tipo_pagamento' => $tpPagTef != null ? $tpPagTef : $p['tipo'],
                                'local_id' => $caixa->local_id,
                                'caixa_id' => $caixa->id,
                        ...
                        for ($i = 0; $i < sizeof($request->tipo_pagamento_row); $i++) {
                            ...
                            if(strtotime($vencimento) > strtotime($dataAtual)){
                                ContaReceber::create([
                                    ...
                                    'valor_recebido' => 0,
                                    'status' => 0,
                                    'descricao' => 'Venda PDV #' . $nfce->numero_sequencial . " Parcela " . $contParcela . " de " . sizeof($request->tipo_pagamento_row),
                                    'empresa_id' => $request->empresa_id,
                                    'juros' => 0,
                                    'multa' => 0,
                ...
                if (isset($request->ordem_servico_id)) {
                    $ordemServico = OrdemServico::findOrfail($request->ordem_servico_id);

                    $ordemServico->nfce_id = $nfce->id;
                    $ordemServico->save();
                }
                ...
                foreach($request->fatura as $key => $f){
                    ...
                    if($f->data && strtotime($vencimento) > strtotime($dataAtual)){
                        ...
                        ContaReceber::create([
                            ...
                            'descricao' => 'Venda PDV #' . $nfce->numero_sequencial . " Parcela " . ($contParcela) . " de " . sizeof($request->fatura),
                            'empresa_id' => $request->empresa_id,
                            'juros' => 0,
                            'multa' => 0,
                            'observacao' => $request->obs_row[$i] ?? '',
    ...
    try {
        ...
        $nfe = DB::transaction(function () use ($request) {
            ...
            if(isset($request->venda_temporaria_id) && $request->venda_temporaria_id){
                ...
                if($vendaTemporaria){
                    $vendaTemporaria->estado = 'finalizada';
                    $vendaTemporaria->tabela = 'nves';
                    $vendaTemporaria->total = $nfe->total;
                    $vendaTemporaria->venda_vinculada = $nfe->id;
                    $vendaTemporaria->save();
                }
            }
            if($request->produto_id){
            ...
            $planoContaId = null;
            if($nfe->natureza->planoConta){
                $planoContaId = $nfe->natureza->planoConta->id;
            }
                    ...
                    foreach($pagamentos as $p){
                        ...
                        if(strtotime($vencimento) > strtotime($dataAtual)){
                            ContaReceber::create([
                                ...
                                'valor_recebido' => 0,
                                'status' => 0,
                                'descricao' => 'Venda Pedido ' . $nfe->numero_sequencial . " Parcela " . $contParcela . " de " . sizeof($pagamentos),
                                'empresa_id' => $request->empresa_id,
                                'juros' => 0,
                                'multa' => 0,
                                'observacao' => '',
...
public function update(Request $request, $id){
    try{
        ...
        $nfce = DB::transaction(function () use ($request, $id) {
            ...
            $planoContaId = null;
            if($item->natureza->planoConta){
                $planoContaId = $item->natureza->planoConta->id;
            }
...
public function qrCodePix(Request $request){
    ...
    if(
        $config == null ||
        (
            (!$config->mercadopago_access_token_pix || !$config->mercadopago_public_key_pix)
            && !$config->asaas_token
        )
    ){
        return response()->json("Configuração de caixa não cadastrada credenciais de PIX", 401);
    }
    try{
        if($config->asaas_token){
            ...
            try{
                $response = $client->request('POST', $endPoint, [
                    'body' => '{"value":'.$request->total_venda.'}',
                    'headers' => [
...
public function consultaPix(Request $request){
    ...
    if($config->asaas_token){
        ...
        if(isset($data['data'])){
            $data = $data['data'];
            foreach($data as $d){
                if($d['conciliationIdentifier'] == $request->payment_id){
...
public function finalizarVenda4(Request $request)
{
    try {

        if (!$request->itens || !is_array($request->itens) || count($request->itens) == 0) {
            return response()->json('Carrinho vazio!', 401);
        }

        if (!$request->pagamentos || !is_array($request->pagamentos) || count($request->pagamentos) == 0) {
            return response()->json('Nenhuma forma de pagamento informada!', 401);
        }
        ...
        $nfce = DB::transaction(function () use ($request) {
            ...
            $config = __objetoParaEmissao($empresa, $caixa->local_id);
            ...
            // $tipoPagamentoPrincipal = $this->mapTipoPagamentoPDV4($request->pagamentos[0]['forma_pagamento'] ?? null);
            $tipoPagamentoPrincipal = $request->pagamentos[0]['codigo_pagamento'] ?? null;
            ...
            $dadosNfce = [
                ...
                'local_id' => $caixa->local_id,
                'observacao' => $request->observacao ?? '',
                'dinheiro_recebido' => $request->total_pago ?? 0,
                ...
                'cAut_cartao' => '',
                'user_id' => $request->usuario_id,
                'valor_entrega' => $request->valor_entrega ?? 0,
                'numero_sequencial' => $this->getLastNumero($request->empresa_id)
            ];

            $nfce = Nfce::create($dadosNfce);
            ...
            foreach ($request->itens as $item) {
                ...
                $subTotal = (float) ($item['sub_total'] ?? ($quantidade * $valorUnitario));
                ...
                if ($produto->gerenciar_estoque) {
                    $this->util->reduzEstoque($produto->id, $quantidade, $variacao_id, $caixa->local_id);
                ...
                if ($produto->prazo_garantia > 0 && $nfce->cliente_id != null) {
                    Garantia::create([
                        'empresa_id' => $request->empresa_id,
                        'cliente_id' => $nfce->cliente_id,
                        ...
                        'nfce_id' => $nfce->id,
                        'usuario_id' => $request->usuario_id,
                        'prazo_garantia' => $produto->prazo_garantia,
                        'data_venda' => date('Y-m-d')
                    ]);
                }
            }
            ...
            $planoContaId = null;
            if ($nfce->natureza && $nfce->natureza->planoConta) {
                $planoContaId = $nfce->natureza->planoConta->id;
            }
            ...
            foreach ($request->pagamentos as $p) {
                ...
                // $tipoPagamento = $this->mapTipoPagamentoPDV4($p['forma_pagamento'] ?? null);
                $tipoPagamento = $p['codigo_pagamento'] ?? null;
                $valorPagamento = (float) ($p['valor'] ?? 0);
                $vencimento = !empty($p['data_vencimento']) ? $p['data_vencimento'] : date('Y-m-d');
                ...
                if (strtotime($vencimento) > strtotime(date('Y-m-d'))) {
                    ContaReceber::create([
                        ...
                        'data_vencimento' => $vencimento,
                        'data_recebimento' => $vencimento,
                        'valor_integral' => $valorPagamento,
                        'valor_recebido' => 0,
                        'status' => 0,
                        'descricao' => 'Venda PDV #' . $nfce->numero_sequencial . " Parcela " . $contParcela . " de " . count($request->pagamentos),
                        'empresa_id' => $request->empresa_id,
                        'juros' => 0,
                        'multa' => 0,
                        'observacao' => '',
            ...
            if (!empty($request->venda_suspensa_id)) {
                $vendaSuspensa = VendaSuspensa::find($request->venda_suspensa_id);
                if ($vendaSuspensa) {
                    $vendaSuspensa->itens()->delete();
                    $vendaSuspensa->delete();
                }
            ...
            // se quiser usar depois no front
            if (!empty($request->acao_finalizacao)) {
                $obs = trim(($nfce->observacao ?? '') . ' | Ação: ' . $request->acao_finalizacao);
                $nfce->observacao = $obs;
                $nfce->save();
            }
...
$result = app(FiscalValidator::class)->validate($nfce, $nfce->empresa);
...
__createLog(
    ...
    'PDV',
    'cadastrar',
    "#$nfce->numero_sequencial - R$ " . __moeda($nfce->total)
);
...
return response()->json([
    ...
    'fiscal_status' => $nfce->fiscal_status,
    'fiscal_risco' => $nfce->fiscal_risco,
    'acao_finalizacao' => $request->acao_finalizacao
], 200);
...
private function mapTipoPagamentoPDV4($forma)
{
...
public function finalizarVendaNfe4(Request $request)
{
    try {

        if (!$request->itens || !is_array($request->itens) || count($request->itens) == 0) {
            return response()->json(['message' => 'Carrinho vazio!'], 401);
        }

        if (!$request->pagamentos || !is_array($request->pagamentos) || count($request->pagamentos) == 0) {
            return response()->json(['message' => 'Nenhuma forma de pagamento informada!'], 401);
        }

        if (!$request->cliente_id) {
            return response()->json(['message' => 'Para emitir NF-e é obrigatório informar o cliente.'], 401);
        }
        ...
        $nfe = DB::transaction(function () use ($request) {
            ...
            if (!$caixa) {
                throw new \Exception("Caixa fechado, abra para salvar a venda!");
            }

            $local_id = $request->local_id ? $request->local_id : $caixa->local_id;

            $empresaEmissao = __objetoParaEmissao($empresa, $local_id);
            ...
            $valorProdutos = 0;
            foreach ($request->itens as $item) {
                $valorProdutos += (float)($item['sub_total'] ?? 0);
            }
            ...
            $dadosNfe = [
                ...
                'numero_serie' => $empresaEmissao->numero_serie_nfe ? $empresaEmissao->numero_serie_nfe : 0,
                'numero_sequencial' => $this->getLastNumeroNfe($request->empresa_id),
                'numero' => $numero,
                'estado' => 'novo',
                ...
                'valor_produtos' => (float)$valorProdutos,
                'valor_frete' => (float)($request->valor_frete ?? 0),
            ...
            foreach ($request->itens as $item) {
                ...
                $variacao_id = $item['variacao_id'] ?? null;
                $cfop = $product->cfop_estadual;
                if($cliente->cidade->uf != $empresa->cidade->uf){
                    $cfop = $product->cfop_outro_estado;
                }
                $itemNfe = ItemNfe::create([
                    ...
                    'perc_icms' => (float)($product->perc_icms ?? 0),
                    'perc_pis' => (float)($product->perc_pis ?? 0),
                    'perc_cofins' => (float)($product->perc_cofins ?? 0),
                    'perc_ipi' => (float)($product->perc_ipi ?? 0),
                    'cst_csosn' => $product->cst_csosn,
                    'cst_pis' => $product->cst_pis,
                    'cst_cofins' => $product->cst_cofins,
                    'cst_ipi' => $product->cst_ipi,
                    'perc_red_bc' => (float)($product->perc_red_bc ?? 0),
                    'cfop' => $cfop,
                    'ncm' => $product->ncm,
                ...
                if (
                    $product->gerenciar_estoque &&
                    $nfe->natureza &&
                    $nfe->natureza->movimentar_estoque == 1
                ) {
            ...
            $planoContaId = null;
            if ($nfe->natureza && $nfe->natureza->planoConta) {
                $planoContaId = $nfe->natureza->planoConta->id;
            }

            foreach ($request->pagamentos as $i => $p) {
                // $tipoPagamento = $this->mapTipoPagamentoPDV4($p['forma_pagamento'] ?? null);
                $tipoPagamentoPrincipal = $p['codigo_pagamento'] ?? null;

                $valorPagamento = (float)($p['valor'] ?? 0);
                $dataVencimento = !empty($p['data_vencimento']) ? $p['data_vencimento'] : date('Y-m-d');
                ...
                ContaReceber::create([
                    ...
                    'data_vencimento' => $dataVencimento,
                    'local_id' => $local_id,
                    'descricao' => 'Parcela ' . ($i + 1) . ' de ' . count($request->pagamentos),
                    'referencia' => 'Pedido ' . $nfe->numero_sequencial,
                    'plano_conta_id' => $planoContaId
                ]);
            }
                ...
                if ($funcionario) {
                    ...
                    if ($valorRetorno > 0) {
                        ComissaoVenda::create([
                            ...
                            'tabela' => 'nfe',
                            'valor' => $valorRetorno,
                            'valor_venda' => (float)($request->total ?? 0),
                            'status' => 0,
                            'empresa_id' => $request->empresa_id
                        ]);
                    }
...
$result = app(FiscalValidator::class)->validate($nfe, $nfe->empresa);
...
__createLog(
    ...
    'Venda',
    'cadastrar',
    "#$nfe->numero_sequencial - R$ " . __moeda($nfe->total)
);
...
return response()->json([
    ...
    'total' => $nfe->total,
    'fiscal_status' => $nfe->fiscal_status,
    'fiscal_risco' => $nfe->fiscal_risco,
    'acao_finalizacao' => $request->acao_finalizacao
], 200);
...
public function opcoesCrediario(Request $request)
{
    $empresaId = $request->empresa_id;
    $valor = __convert_value_bd($request->valor);

    $config = ConfiguracaoCrediario::where('empresa_id', $empresaId)
    ->where('ativo', 1)
    ->where('valor_minimo', '<=', $valor)
    ->where(function($q) use ($valor){
        $q->whereNull('valor_maximo')->orWhere('valor_maximo', '>=', $valor);
    })
    ->orderByDesc('valor_minimo')
    ->first();
    ...
    if(!$config){
        ...
        for($i = 1; $i <= 12; $i++){
            ...
            $opcoes[] = [
                ...
                'valor_total' => $total,
                'juros' => 0,
                'primeiro_vencimento' => $vencimento->format('Y-m-d'),
                'intervalo' => 30,
                'padrao' => true
            ];
        }
    ...
    for($i = 1; $i <= $config->maximo_parcelas; $i++){

        $juros = $i > $config->parcelas_sem_juros ? $config->juros_percentual : 0;
        $total = $valor + ($valor * ($juros / 100));
        $valorParcela = $total / $i;

        $vencimento = Carbon::now()->addDays($config->primeiro_vencimento_dias);
        ...
        $opcoes[] = [
            ...
            'juros' => $juros,
            'primeiro_vencimento' => $vencimento->format('Y-m-d'),
            'intervalo' => $config->intervalo_parcelas_dias
        ];
    }
