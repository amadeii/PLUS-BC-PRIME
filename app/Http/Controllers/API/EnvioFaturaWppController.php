...
class EnvioFaturaWppController extends Controller
{
    ...
    public function createFilesOrcamento(Request $request){
        $id = $request->id;
        $tipo = $request->tipo;
        ...
        if($request->enviar_pedido_a4 == true){
            $pedidoA4 = $this->criaPedidoOrmcamento($item, $tipo);
            $data['pedido_a4_link'] = $pedidoA4;
            $mensagem .= "*Link do orçamento* " . $pedidoA4 . "%0A";
        }

        return response()->json($mensagem, 200);
    }

    public function criaPedidoOrmcamento($item)
    {
        $config = Empresa::where('id', $item->empresa_id)->first();
    ...
    public function createFilesOrdemServico(Request $request){
        $id = $request->id;
        $tipo = $request->tipo;
        ...
        if($request->enviar_pedido_a4 == true){
            ...
            $pedidoA4 = $this->criaPedidoOrdemServico($item, $tipo);
            $data['pedido_a4_link'] = $pedidoA4;
            $mensagem .= "*Link da Ordem de Serviço* " . $pedidoA4 . "%0A";
        }
    ...
    public function criaPedidoOrdemServico($ordem)
    {

        $config = Empresa::where('id', $ordem->empresa_id)->first();
        if ($config == null) {
            die;
        }
