...
class ProdutoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($padraoTributacaoProduto == null) {
            return response()->json(['message' => 'Defina um padrão de tributação primeiro!'], 401);
        }
        ...
        $localPadrao = Localizacao::where('status', 1)
        ...
        if ($request->estoque_inicial) {
            ...
            if ($funcionario && $funcionario->usuario) {
                $this->utilEstoque->incrementaEstoque($produto->id, $request->estoque_inicial, null, $localPadrao->id);

                $transacao = Estoque::where('produto_id', $produto->id)
                ->orderBy('id', 'desc')
                ->first();
    ...
    public function upload(Request $request)
    {
        if (!$request->hasFile('image')) {
            return response()->json(['message' => 'Nenhuma imagem enviada'], 401);
        }

        if (!$request->produto_id) {
            return response()->json(['message' => 'produto_id é obrigatório'], 422);
        }
