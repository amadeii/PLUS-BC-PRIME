...
class OrdemServicoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $envioWppLink = 0;
        if($configGeral != null && $configGeral->status_wpp_link){
            $envioWppLink = 1;
        }
        return view('ordem_servico.index', compact('data', 'convenios', 'veiculos', 'configGeral', 'envioWppLink'));
    }
    ...
    public function create()
    {
        ...
        return view('ordem_servico.create', compact('hoje', 'funcionario', 'usuario', 'servicos', 'convenios', 
            'formatosArmacao', 'tiposArmacao', 'tratamentos', 'veiculos', 'configGeral', 'funcionarios'));
    }
    ...
    public function storeProduto(Request $request)
    {
        ...
        try {
            ...
            $valorUnitario = __convert_value_bd($request->valor_produto);
            ...
            $this->recalculaValorOrdemServico($ordem);

            $descricaoLog = "#$ordem->codigo_sequencial - Produto Adicionado: " . $produto->produto->nome;
    ...
    private function recalculaValorOrdemServico($ordem)
    {
        $ordem = OrdemServico::findOrFail($ordem->id);

        $totalProdutos = $ordem->itens->sum('subtotal');
        $totalServicos = $ordem->servicos->sum('subtotal');

        $percentual = $ordem->percentual_desconto_produtos ?? 0;
        $valorDesconto = ($totalProdutos * $percentual) / 100;

        $ordem->valor = ($totalServicos + $totalProdutos) - $valorDesconto;
        $ordem->save();
    }
    ...
    public function updateDescontoProdutos(Request $request, $id)
    {
        $ordem = OrdemServico::findOrFail($id);
        __validaObjetoEmpresa($ordem);

        try {
            $percentual = __convert_value_bd($request->percentual_desconto_produtos ?? 0);
            ...
            $ordem->percentual_desconto_produtos = $percentual;
            $ordem->save();

            $this->recalculaValorOrdemServico($ordem);

            session()->flash("flash_success", "Desconto dos produtos atualizado!");
        } catch (\Exception $e) {
            session()->flash("flash_error", "Algo deu errado: " . $e->getMessage());
        }
    ...
    public function gerarNfe($id)
    {
        ...
        $configGeral = ConfigGeral::where('empresa_id', request()->empresa_id)->first();
        $cadastroSimplificado = $configGeral && $configGeral->cadastro_simplificado_cliente;

        $totalProdutos = $item->itens->sum('subtotal');
        $percentualDescontoProdutos = $item->percentual_desconto_produtos ?? 0;
        $valorDescontoProdutos = ($totalProdutos * $percentualDescontoProdutos) / 100;
        $item->desconto = round($valorDescontoProdutos, 2);

        return view('nfe.create', compact('item', 'cidades', 'transportadoras', 'naturezas', 'isOrdemServico', 'numeroNfe', 'funcionarios',
            'naturezaPadrao', 'cadastroSimplificado'));
    }

    public function gerarPdv($id)
    {
        $ordem = OrdemServico::findOrFail($id);
        __validaObjetoEmpresa($ordem);

        if($ordem->nfe_id != 0){
            session()->flash("flash_warning", "Esta OS já possui venda gerada!");
            return redirect()->back();
        }
        ...
        $itens = $ordem->itens;
        foreach($itens as $i){
            $i->valor_unitario = $i->quantidade > 0 ? $i->subtotal / $i->quantidade : 0;
        }

        $servicos = $ordem->servicos;
        foreach($servicos as $s){
            $s->valor = $s->quantidade > 0 ? $s->subtotal / $s->quantidade : 0;
            $s->valor_unitario = $s->valor;
        }
        ...
        $valorDescontoProdutos = ($totalProdutos * $percentualDescontoProdutos) / 100;
        $desconto = round($valorDescontoProdutos, 2);
        $servicos = [];
