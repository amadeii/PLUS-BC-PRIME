...
class ContadorController extends Controller
{
    ...
    {
        $item = Empresa::findOrFail($id);
        return view('contadores.white_label', compact('item'));
    }

    public function saveWhiteLabel(Request $request, $id)
    {
        ...
        if($request->hasFile('logo_login')){
            if($item->logo_login && file_exists(public_path('uploads/white-label/'.$item->logo_login))){
                unlink(public_path('uploads/white-label/'.$item->logo_login));
            }

            $item->logo_login = $this->util->uploadImage($request, '/white-label', 'logo_login');
        }

        if($request->hasFile('logo_sidebar')){
            if($item->logo_sidebar && file_exists(public_path('uploads/white-label/'.$item->logo_sidebar))){
                unlink(public_path('uploads/white-label/'.$item->logo_sidebar));
            }

            $item->logo_sidebar = $this->util->uploadImage($request, '/white-label', 'logo_sidebar');
        }

        $item->save();

        session()->flash('flash_success', 'White Label atualizado com sucesso!');
        return redirect()->back();
    }
