...
class FrontBoxController extends Controller
{
    ...
    {
        $config = ConfigGeral::where('empresa_id', request()->empresa_id)->first();
        if($config != null && $config->modelo == 'flash'){
            session()->reflash();
            return redirect()->route('frontbox.create4');
        }
        if (!__isCaixaAberto()) {
        ...
        if($config != null && $config->modelo == 'quick'){
            ...
            // ->limit(50) //remover depopis so teste
            ->get();
        ...
        if($caixa != null){
            $vendaTemporaria = VendaTemporaria::where('usuario_id', get_id_user())
            ->with(['itens', 'cliente'])
            ->where('estado', 'em_aberto')->first();
            if($vendaTemporaria == null){
                $vendaTemporaria = VendaTemporaria::create([
        ...
        return view($view, compact('categorias', 'abertura', 
            ...
            'clientePadrao', 'isOrcamento', 'funcionario', 'contasEmpresa', 'configuracaoTef', 'vendaTemporaria', 'configuracaoTefScope', 
            'cadastroSimplificado', 'vendedor'));
    }
    ...
    public function create4(Request $request)
    {
        ...
        return view('front_box.create4', compact('funcionarios', 'tiposPagamento', 'listasPreco', 'configuracaoTef', 'configuracaoTefScope'));
    }
    ...
    public function edit(string $id)
    {
        ...
        return view($view, compact('categorias', 'abertura', 'funcionarios', 'item', 'cliente', 'funcionario', 
            ...
            'user', 'clientes', 'listasPreco', 'itensVenda', 'title', 'isOrcamento', 'configuracaoTef'));
    }
