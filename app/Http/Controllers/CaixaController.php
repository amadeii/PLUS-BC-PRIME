...
use App\Models\User;
use App\Models\CaixaFormaPagamento;
...
class CaixaController extends Controller
{
        ...
        try {
            ...
            if($request->conta_empresa_id){
                ...
                $data = [
                    ...
                    'descricao' => "Abertura de caixa - " . ($request->observacao ?? ""),
                    'tipo_pagamento' => '01',
                    'valor' => __convert_value_bd($request->valor_abertura),
                    'caixa_id' => $item->id,
                    'tipo' => 'saida'
                ];
    ...
    public function fechar(Request $request)
    {
        ...
        // Limpa os tipos antigos antes de recriar
    ...
    public function imprimir($id)
    {
        ...
        $data = $this->agrupaDados($nfce, $nfe, $ordens, $compras);
        $somaTiposPagamento = $this->somaTiposPagamentoFechar($data, $item);
    ...
    public function imprimir80($id)
    {
        ...
        $produtos = $this->totalizaProdutos($data);
        return view('caixa.imprimir_80', compact(
            'item',
            'data',
            'usuario',
            'totalVendas',
    ...
    private function somaTiposPagamentoFechar($vendas, $caixa)
    {
        $tipos = $this->preparaTipos();
        ...
        $contasReceber = ContaReceber::where('caixa_id', $caixa->id)
        ->where('tipo_pagamento', '01')
        ->where('status', 1)->sum('valor_recebido');

        $contasPagar = ContaPagar::where('caixa_id', $caixa->id)
        ->where('tipo_pagamento', '01')
        ->where('status', 1)->sum('valor_pago');
        $tipos['01'] += $caixa->valor_abertura - $sangrias + $contasReceber - $contasPagar;
