...
class EmpresaUtil
{
	...
	private function initProducts($empresa_id): void
	{
		$localizacao = Localizacao::where('empresa_id', $empresa_id)->first();
		...
		->select('id')
		->chunkById(500, function ($produtos) use ($localizacao) {

			$produtoIds = $produtos->pluck('id');

			$existentes = ProdutoLocalizacao::where('localizacao_id', $localizacao->id)
			->whereIn('produto_id', $produtoIds)
			->pluck('produto_id')
			->toArray();

            // Monta apenas os que faltam
			$insert = [];

			foreach ($produtoIds as $produtoId) {
				if (!in_array($produtoId, $existentes)) {
					$insert[] = [
			...
			if (!empty($insert)) {
				ProdutoLocalizacao::insert($insert);
			}
		});
	}
