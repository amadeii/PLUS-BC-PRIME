...
// function __casas_decimais_valor()
// {
// 	try{
// 		if(!Auth::user()->empresa){
// 			return 2;
...
// 			return 2;
// 		}
// 		return $config->casas_decimais_valor;
// 	}catch(\Exception $e){
// 		return 2;
// 	}
// }
...
{
	try{
		$empresa_id = session('empresa_id') ?? request()->empresa_id;

		if(!$empresa_id && Auth::check() && Auth::user()->empresa){
			$empresa_id = Auth::user()->empresa->empresa_id;
		}
		...
		return $config->casas_decimais_valor ?? 2;
...
function __moeda($valor, $casas_decimais = null)
{
	return number_format($valor, $casas_decimais ?? __casas_decimais_valor(), ',', '.');
}

function __moedaInput($valor, $casas_decimais = null)
{
	return number_format($valor, $casas_decimais ?? __casas_decimais_valor(), ',', '');
}
...
function __objetoParaEmissao($empresa, $local_id){
	...
	$localizacoes = Localizacao::where('empresa_id', $empresa->id)
	->where('status', 1);
	...
	$localizacao = Localizacao::where('empresa_id', $empresa->id)
	->where('status', 1)
	->where('id', $local_id)
	->first();
	...
	if($primeiraLocalizacao && $primeiraLocalizacao->id == $localizacao->id){
		return $empresa;
	}
...
if (!function_exists('auditoriaDiff')) {
	function auditoriaDiff(array $antes, array $depois): array
	{
		$alteracoes = [];

		$todosCampos = array_unique(array_merge(array_keys($antes), array_keys($depois)));
...
if (!function_exists('auditoriaLimparDados')) {
...
if (!function_exists('auditoriaSalvar')) {
	function auditoriaSalvar(
		string $entidade,
		int $registroId,
		string $acao,
		?array $antes = null,
		...
		$antes = auditoriaLimparDados($antes);
		$depois = auditoriaLimparDados($depois);

		AuditoriaRegistro::create([
			'empresa_id' => $empresaId ?? ($depois['empresa_id'] ?? $antes['empresa_id'] ?? null),
			'usuario_id' => Auth::user()->id,
			'entidade' => $entidade,
			...
			'antes_json' => $antes,
			'depois_json' => $depois,
			'alteracoes_json' => auditoriaDiff($antes ?? [], $depois ?? []),
			'ip' => request()->ip(),
...
if (!function_exists('auditoriaLabelCampo')) {
	function auditoriaLabelCampo($campo)
	{
		$labels = [
			'nome' => 'Nome',
			'referencia' => 'Referência',
			'codigo_barras' => 'Código de barras',
			'codigo_barras2' => 'Código de barras 2',
			'codigo_barras3' => 'Código de barras 3',
			'unidade' => 'Unidade',
			'imagem' => 'Imagem',
			'descricao' => 'Descrição',

			'valor_unitario' => 'Valor de venda',
			'valor_compra' => 'Valor de compra',
			'valor_prazo' => 'Valor a prazo',
			'valor_minimo_venda' => 'Valor mínimo venda',
			'valor_atacado' => 'Valor atacado',
			'quantidade_atacado' => 'Qtd atacado',
			'percentual_lucro' => 'Percentual lucro',

			'gerenciar_estoque' => 'Gerenciar estoque',
			'estoque_minimo' => 'Estoque mínimo',
			'alerta_validade' => 'Alerta validade',
			'local_armazenamento' => 'Local armazenamento',

			'ncm' => 'NCM',
			'cfop_estadual' => 'CFOP estadual',
			...
			'perc_cofins' => '% COFINS',
			'perc_ipi' => '% IPI',
			'perc_red_bc' => '% Redução BC',
			'codigo_beneficio_fiscal' => 'Benefício fiscal',

			'cardapio' => 'Cardápio',
			'delivery' => 'Delivery',
			'reserva' => 'Reserva',
			'valor_cardapio' => 'Valor cardápio',
			'valor_delivery' => 'Valor delivery',
			'tempo_preparo' => 'Tempo preparo',
			'destaque_delivery' => 'Destaque delivery',
			'oferta_delivery' => 'Oferta delivery',
			'destaque_cardapio' => 'Destaque cardápio',
			'oferta_cardapio' => 'Oferta cardápio',

			'combo' => 'Produto é combo',
			'composto' => 'Produto composto',
			'lancar_produtos_combo' => 'Lançar produtos combo',
			'margem_combo' => 'Margem combo',

			'status' => 'Status',
			'numero_sequencial' => 'Número sequencial',
			'observacao' => 'Observação',
			'observacao2' => 'Observação 2',
			'observacao3' => 'Observação 3',
			'observacao4' => 'Observação 4',
		];
...
if (!function_exists('auditoriaValor')) {
	function auditoriaValor($valor)
	{
		if (is_null($valor)) return '--';
		if (is_bool($valor)) return $valor ? 'Sim' : 'Não';
		if (is_numeric($valor) && str_contains((string)$valor, '.')) {
			return 'R$ ' . __moeda($valor, 2);
		}
		if (is_array($valor)) return json_encode($valor, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
		return (string)$valor;
	}
}

function __getBancosAtivos($empresaId)
{
	$bancos = ['' => 'Selecione'];

	$configs = [
		'sicredi' => [
			'model' => \App\Models\SicrediConfig::class,
			'campo' => 'status',
			'label' => 'Sicredi'
		],
		'asaas' => [
			'model' => \App\Models\AsaasConfig::class,
			'campo' => 'status',
			'label' => 'Asaas'
		],
	];
	...
	foreach ($configs as $key => $config) {
		if ($config['model']::where('empresa_id', $empresaId)->where($config['campo'], 1)->exists()) {
			$bancos[$key] = $config['label'];
		}
	}
...
if (!function_exists('__appHealthPing')) {
	function __appHealthPing($rota = null, $extra = [])
	{
		try {

			if (!Auth::check()) return;

			$key = 'app_health_ping_daily_' . env('APP_URL', gethostname());

			if (Cache::has($key)) return;

			$url = 'https://slymlicenca.com.br/api/logs';
			$token = env('APP_HEALTH_TOKEN', 'SLYM_2026_@Monitor_837461923');
			...
			->post($url, [
				'origem' => 'WEB',
				'identificador' => env('APP_URL', gethostname()),
				'app_nome' => env('APP_NAME', 'SLYM'),
				'app_versao' => env('APP_BUILD_VERSION', '1.0.0'),
				'rota' => $rota ?? request()->path(),
				'payload' => array_merge([
...
function __whiteLabel()
{
	...
	$empresaAtualId = $user->empresa->empresa_id;

	$vinculo = ContadorEmpresa::with('contador')
	->where('empresa_id', $empresaAtualId)
	->first();

	if ($vinculo && $vinculo->contador) {
		return $vinculo->contador;
	}

	return $user->empresa->empresa ?? null;
}

function __nomeSistema()
{
	return __whiteLabel()?->nome_sistema ?? env('APP_NAME');
}