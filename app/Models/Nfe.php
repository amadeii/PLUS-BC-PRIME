...
class Nfe extends Model
{
    ...
    protected $fillable = [
        ...
        'cidade_id_entrega', 'marca', 'fiscal_status', 'fiscal_risco', 'fiscal_mensagens', 'estado_compra', 'indPres', 'estado_fatura',
        'devolucao'
    ];
    ...
    public function _totalIpi()
    {
        $totalItens = $this->itens->sum('sub_total');
        return $this->total - $totalItens - $this->desconto;
    }
    ...
    {
        return $this->hasOne(OrdemProducao::class, 'orcamento_id');
    }
    ...
    {
        return $this->hasMany(NfeDivergencia::class);
    }

    public function outroEstado()
    {
        if(!$this->cliente || !$this->cliente->cidade) return 0;
