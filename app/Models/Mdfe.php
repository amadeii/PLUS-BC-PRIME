...
class Mdfe extends Model
{
	...
	protected $fillable = [
		...
		'nome_pagador', 'documento_pagador', 'ind_pag', 'valor_transporte', 'importado', 'xml_importado', 'data_importacao',
		'cUnid'
	];
	...
	public static function unidadesCargaMdfe()
	{
		return [
			'01' => 'KG',
			'02' => 'TON',
	...
	public function estadoEmissao()
	{
		if ($this->estado_emissao == 'aprovado') {
			return "<span class='badge bg-success p-1'>Aprovado</span>";
		} else if ($this->estado_emissao == 'cancelado') {
			return "<span class='badge bg-danger p-1'>Cancelado</span>";
		} else if ($this->estado_emissao == 'rejeitado') {
			return "<span class='badge bg-warning p-1'>Rejeitado</span>";
		}
		return "<span class='badge bg-info p-1'>Novo</span>";
	}
}
