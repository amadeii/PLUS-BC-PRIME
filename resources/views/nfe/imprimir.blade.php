...
<body>
    ...
    <table>
        <tr>
            <td class="b-top text-left" style="width: 700px;">
                Endereço: <strong>{{$config->rua}}, {{$config->numero}} - {{$config->bairro}} - {{$config->cidade->info}}</strong>
            </td>
        </tr>
    </table>
    <table>
    ...
    <table>
        <tr>
            <td class="b-top text-left" style="width: 500px;">
                Endereço: <strong>{{$item->fornecedor->rua}}, {{$item->fornecedor->numero}} - {{$item->fornecedor->bairro}} - {{$item->fornecedor->cidade?->info}}</strong>
            </td>
    ...
    <table>
        <tr>
            <td class="b-top text-left" style="width: 500px;">
                Endereço: <strong>{{$item->cliente->rua}}, {{$item->cliente->numero}} - {{$item->cliente->bairro}} - {{$item->cliente->cidade?->info}}</strong>
            </td>
    ...
    <table>
        <thead>
            <tr>
                <td style="width: 65px; text-align: left;">Imagem</td>
                <td style="width: 90px; text-align: left;">Cod/Ref</td>
                <td style="width: 285px; text-align: left;">Descrição</td>
                <td style="width: 75px; text-align: left;">Qtd.</td>
                <td style="width: 75px; text-align: left;">Vl Uni</td>
                <td style="width: 75px; text-align: left;">Vl Liq.</td>
            </tr>
        </thead>
        ...
        <tbody>
            ...
            @php
            $produto = $i->produto;
            $imagemProduto = $produto && $produto->imagem 
            ? public_path('uploads/produtos/' . $produto->imagem) 
            : public_path('imgs/no-image.png');

            $imagemProdutoBase64 = file_exists($imagemProduto)
            ? 'data:image/png;base64,' . base64_encode(file_get_contents($imagemProduto))
            : 'data:image/png;base64,' . base64_encode(file_get_contents(public_path('imgs/no-image.png')));
            @endphp

            <tr>
                <th class="b-top" style="text-align: left; width: 65px;">
                    <img src="{{ $imagemProdutoBase64 }}" style="width: 35px; height: 35px; object-fit: cover; border-radius: 4px; margin-top: 3px;">
                </th>

                <th class="b-top" style="text-align: left; width: 90px;">
                    {{ $produto->numero_sequencial }}
                    {{ $produto->referencia != "" ? "/ " . $produto->referencia : "" }}
                </th>

                <th class="b-top" style="text-align: left; width: 285px;">
                    <strong>{{ $i->descricao() }}</strong>

                    @if($produto->local_armazenamento)
                    <br>
                    <span style="font-size: 10px;">
                        Local de armazenamento: <strong>{{ $produto->local_armazenamento }}</strong>
                    </span>
                    @endif
                </th>

                <th class="b-top" style="text-align: left; width: 75px;">
                    ...
                    <br><span style="font-size: 10px;">x{{ __moeda($i->quantidade_dimensao) }}</span>
                    @endif
                </th>

                <th class="b-top" style="text-align: left; width: 75px;">{{ __moeda($i->valor_unitario) }}</th>
                <th class="b-top" style="text-align: left; width: 75px;">{{ __moeda($i->sub_total) }}</th>
            </tr>
            @php
            $somaItens += $i->quantidade;
            $somaTotalItens += round($i->sub_total, 2);
