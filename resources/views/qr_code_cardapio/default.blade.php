...
<body>
    ...
    <input type="hidden" id="session_cart_cardapio" value="{{ $carrinho->session_cart_cardapio ?? request()->cookie('cardapio_cart_token') }}">
    <input type="hidden" id="session_cart_user" value="{{ $carrinho->session_cart_user ?? request()->cookie('cardapio_user_token') }}">
...
<script>
    ...
    function sacola_count() {
        ...
        .done(function(data) {
            ...
            $(".shop-bag .counter").html(total);
            $(".social .counter").html(total);

            if(total > 0){
                $(".social .counter").css('display', 'inline-block');
            }else{
                $(".social .counter").css('display', 'none');
            }
        });
    }
