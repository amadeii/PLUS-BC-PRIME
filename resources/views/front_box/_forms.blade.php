...
<input type="hidden" id="vendedor" value="{{ isset($vendedor) && $vendedor ? json_encode($vendedor) : null }}" name="">
...
@if(isset($vendaTemporaria))
<input type="hidden" id="venda_temporaria_id" value="{{ $vendaTemporaria->id }}">
@endif
...
@if(isset($isOrdemServicoPdv) && $ordem)
<input type="hidden" value="{{ $ordem->id }}" name="ordem_servico_id">
<input type="hidden" id="ordem_servico_desconto" value="{{ $desconto }}">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="small text-muted ps-1" style="font-size: 0.8rem; line-height: 1.1">
                            ...
                            <span style="margin-left: 30px; display: inline-block;" class="badge bg-primary encontrar_produtos">
                                <i class="ri-search-line me-1 text"></i>Encontrar produtos
                            </span>
                        </div>
        ...
        <div class="card" style="height: 866px">
            ...
            <div class="row m-2">
                ...
                <div class="col-md-1">
                    ...
                    {!! Form::hidden('valor_total', '')->attrs(['class' => '']) !!}
                </div>
            </div>
                ...
                <div data-bs-target="#navbar-example2" class="scrollspy-example" style="height: 570px">
                    <table class="table table-striped dt-responsive nowrap table-itens">
                        ...
                        <tbody>
                                ...
                                <td class="datatable-cell">
                                    <div class="form-group mb-2" style="width: 200px">
                                        <div class="input-group">
                                            ...
                                            <input type="tel" class="form-control qtd decimal3 qtd_row" name="quantidade[]" value="{{ number_format($product->quantidade, 2, ',', '') }}">
                                            <div class="input-group-append">
                            ...
                            @if (isset($isOrdemServicoPdv) && isset($itens))
                            @foreach ($itens as $key => $product)
                            <tr class="line-product">
                                <input readonly type="hidden" name="key" class="form-control" value="{{ $product->key }}">
                                <input readonly type="hidden" name="produto_id[]" class="produto_row" value="{{ $product->produto->id }}">
            ...
            <div class="row m-1">
                ...
                <div class="col-md-4" style="margin-top: 0px;">
                    ...
                    <div class="col-12 mt-3">
                        ...
                        <button type="button" onclick="carregarVendasTEF()" class="btn btn-dark btn-sm mb-1" style="float: right;">
                            <i class="ri-safe-2-fill"></i> Registros de TEF
                        </button>
                        @endif
...
@include('front_box.partials._modal_buscar_produtos', ['not_submit' => true])
@include('front_box.partials._modal_finalizar', ['not_submit' => true])

@include('front_box.partials._modal_recebimento')
@include('front_box.partials._fatura_padrao')
@include('front_box.partials._edit_fatura')
@include('front_box.partials._modal_opcoes_crediario')
...
@include('front_box.partials.modal_kilo_produto')

@include('modals._observacao_pdv')
@include('modals._adicionais_pdv')
@include('front_box.partials._modal_pagamento_sitef')
@include('front_box.partials.modal_tef_operacoes')
@include('front_box.partials.modal_tef_pendencias')
...
<script src="/js/busca_produto.js" type=""></script>
...
@if(isset($configuracaoTef) && $configuracaoTef != null)
<script src="/js/tef.js" type=""></script>
@endif

@if(isset($configuracaoTefScope) && $configuracaoTefScope != null)
<script>
    window.TEF_CONFIG = {
        ip: "{{ $configuracaoTefScope->agente_ip }}",
        porta: "{{ $configuracaoTefScope->agente_porta }}"
    };
</script>
<script src="/js/tef_scope.js" type=""></script>
@endif

<script type="text/javascript">

    @if(isset($vendaTemporaria) && sizeof($vendaTemporaria->itens) > 0)

    swal({
        title: "Venda temporária encontrada",
        text: "Já existe uma venda em andamento. Deseja continuar ou abandonar?",
        icon: "warning",
        buttons: {
            abandonar: {
                text: "Abandonar",
        ...
        if (!value) {
            return abrirModalVendaTemporaria();
        }

        if (value === "continuar") {
            carregarVendaTemporaria(@json($vendaTemporaria));
        }

        if (value === "abandonar") {
            abandonarVendaTemporaria();
        }
